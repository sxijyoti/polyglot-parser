...
int py_adapter(const char *fpath, ir_result *ir) {
    ...
    TSQuery *qfunc = ts_query_new(tree_sitter_python(), func_q, sizeof(func_q) - 1, &err_off, &err_type);
    if (!qfunc) {
        fprintf(stderr, "Python function query error at offset %u (type %d)\n", err_off, (int)err_type);
    } else {
        ...
        while (ts_query_cursor_next_match(cur, &match)) {
            ...
            if (got_name) {
                ...
                ir_symbol *sym = ir_add_symbol(ir, fname, IR_SYMBOL_FUNCTION, "py", fpath, line);
                if (sym && got_params)
    ...
    static const char imp_q[] =
        "[(import_statement (dotted_name) @module)"
        " (import_from_statement module_name: (dotted_name) @module)]";

    TSQuery *q_imp = ts_query_new(tree_sitter_python(), imp_q, sizeof(imp_q) - 1, &err_off, &err_type);
    if (q_imp) {
        TSQueryCursor *cur = ts_query_cursor_new();
        ts_query_cursor_exec(cur, q_imp, root);
        TSQueryMatch match;
        while (ts_query_cursor_next_match(cur, &match)) {
            for (uint16_t c = 0; c < match.capture_count; c++) {
                char mod[256] = {0};
